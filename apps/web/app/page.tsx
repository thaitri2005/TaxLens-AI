...
function CitationLine({ citation, language = "en" }: { citation: Citation; language?: "en" | "vi" }) {
  ...
  return <div className="citation"><span className="tag">{citation.document_number}</span><span>{location || (language === "vi" ? "Đoạn trích nguồn" : "Source passage")}</span>{citation.source_url && <a href={citation.source_url} target="_blank" rel="noreferrer">{language === "vi" ? "Mở nguồn chính thức ↗" : "Official source ↗"}</a>}</div>;
}
...
export default function Home() {
  ...
  const copy = language === "vi" ? {
    search: "Tìm kiếm", ask: "Hỏi TaxLens", compare: "So sánh phiên bản", documents: "Văn bản",
    heroEyebrow: "Trợ lý tra cứu thuế Việt Nam", heroTitleOne: "Tìm đúng quy định.", heroTitleTwo: "Nắm rõ bằng chứng.",
    heroCopy: "Tra cứu văn bản chính thức, đặt câu hỏi có căn cứ và so sánh các phiên bản quy định trong một không gian làm việc.",
    ...
    askEyebrow: "Trả lời dựa trên bằng chứng", askTitle: "Hỏi TaxLens", citedResponse: "Câu trả lời có trích dẫn",
    askPlaceholder: "Đặt câu hỏi về quy định thuế Việt Nam…", askButton: "Gửi câu hỏi →", pressEnter: "Nhấn Enter để gửi câu hỏi",
    compareEyebrow: "Thông tin phiên bản", compareTitle: "So sánh quy định", compareChip: "Đối chiếu điều khoản",
    before: "Văn bản trước", after: "Văn bản sau", compareButton: "So sánh phiên bản →",
    ...
    adjustFilters: "Hãy điều chỉnh bộ lọc nguồn hoặc loại văn bản.", noDocs: "Chưa có văn bản được lập chỉ mục",
    noDocsText: "Nạp văn bản chính thức để xây dựng thư viện nguồn.", working: "Đang xử lý…",
    evidencePassage: "đoạn bằng chứng", evidencePassages: "đoạn bằng chứng", relevantPassage: "Đoạn liên quan",
    noResults: "Không tìm thấy đoạn phù hợp", broaden: "Hãy thử câu hỏi rộng hơn bằng tiếng Việt hoặc tiếng Anh.",
    searchCorpus: "Tìm kiếm trong kho văn bản", searchHelp: "Mỗi văn bản xuất hiện một lần cùng các đoạn bằng chứng nổi bật.",
    chooseVersions: "Chọn hai phiên bản văn bản", chooseVersionsText: "Dùng số hiệu văn bản trong mục Văn bản để xem các điều khoản được thêm, xóa hoặc sửa đổi.",
    noChanges: "Không phát hiện thay đổi điều khoản", noChangesText: "Hai phiên bản được chọn không có khác biệt điều khoản rõ ràng.",
  } : {
  ...
  return <>
    ...
    <main className={`shell ${language === "vi" ? "language-vi" : ""}`}>
      ...
      {view === "documents" && <section className="view">{selectedDocument ? <><button className="back-button" onClick={() => setSelectedDocument(null)}>← {language === "vi" ? "Quay lại danh sách" : "Back to documents"}</button>{documentBusy ? <Empty title={copy.working} text={language === "vi" ? "Đang đọc phiên bản và các đoạn đã lập chỉ mục." : "Reading versions and indexed passages."} /> : documentError ? <div className="error-state">{documentError}</div> : documentDetail && <DocumentDetailView detail={documentDetail} chunks={documentChunks} language={language} />}</> : <><Heading eyebrow={copy.documentsEyebrow} title={copy.documentsTitle} chip={`${visibleDocuments?.length || 0} ${language === "vi" ? "đã lập chỉ mục" : "indexed"}`} /><div className="form-row"><label>{copy.source}<select value={documentSourceFilter} onChange={event => setDocumentSourceFilter(event.target.value)}><option value="all">{copy.allSources}</option>{Array.from(new Set(documents?.map(document => document.source_name).filter(Boolean))).map(source => <option key={source} value={source || ""}>{source}</option>)}</select></label><label>{copy.type}<select value={documentTypeFilter} onChange={event => setDocumentTypeFilter(event.target.value)}><option value="all">{copy.allTypes}</option>{Array.from(new Set(documents?.map(document => document.document_type))).map(type => <option key={type} value={type}>{type}</option>)}</select></label></div>{documentError ? <div className="error-state">{documentError}</div> : documents === null ? <Empty title={copy.working} text={language === "vi" ? "Đang đọc thư viện văn bản pháp luật." : "Reading the indexed legal documents."} /> : <div className="document-list">{visibleDocuments?.length ? visibleDocuments.map(document => <button className="document-card document-button" key={document.document_number} onClick={() => openDocument(document)}><div className="doc-top"><span className="doc-number">{document.document_number}</span><span className="tag">{document.document_type}</span></div><strong>{document.title}</strong><div className="doc-meta">{document.issuing_agency || (language === "vi" ? "Chưa có cơ quan ban hành" : "Agency unavailable")}<span>{document.source_name || (language === "vi" ? "Chưa có nguồn" : "Source unavailable")} · {language === "vi" ? "Mở chi tiết →" : "Open details →"}</span></div></button>) : <Empty title={copy.noMatch} text={copy.adjustFilters} />}</div>}</>}</section>}
...
function DocumentDetailView({ detail, chunks, language }: { detail: DocumentDetail; chunks: Chunk[]; language: "en" | "vi" }) {
  return <>
    <Heading eyebrow="Indexed source" title={detail.document_number} chip={detail.document_type} />
    <p className="document-title">{detail.title}</p>
    <p className="document-source">{detail.source_name || (language === "vi" ? "Chưa có nguồn" : "Source unavailable")} · {detail.issuing_agency || (language === "vi" ? "Chưa có cơ quan ban hành" : "Agency unavailable")} {detail.versions[0]?.source_url && <a href={detail.versions[0].source_url} target="_blank" rel="noreferrer">{language === "vi" ? "Mở nguồn chính thức ↗" : "Open official source ↗"}</a>}</p>
    <div className="version-grid">{detail.versions.map(version => <div className="version-card" key={version.id}><span className="tag">{version.legal_status}</span><strong>Version {version.issue_date || "date unavailable"}</strong><small>Effective: {version.effective_date || "not recorded"}</small><small>{version.processing_status || "NOT_PROCESSED"} · {version.chunk_count} chunks</small>{version.processing_error_code && <small>{version.processing_error_code}</small>}</div>)}</div>
    <div className="chunk-heading"><p className="eyebrow">{language === "vi" ? "Đoạn văn đã lập chỉ mục" : "Indexed passages"}</p><span>{chunks.length} {language === "vi" ? "đoạn" : "chunks"}</span></div>
    {chunks.length ? <div className="chunk-list">{chunks.map(chunk => <article className="chunk-card" key={chunk.id}><div className="chunk-top"><strong>{chunk.article_number ? `${language === "vi" ? "Điều" : "Article"} ${chunk.article_number}` : (language === "vi" ? "Đoạn văn bản" : "Document passage")}</strong><span>{chunk.page_start ? `${language === "vi" ? "Trang" : "Page"} ${chunk.page_start}${chunk.page_end && chunk.page_end !== chunk.page_start ? `–${chunk.page_end}` : ""}` : (language === "vi" ? "Chưa có số trang" : "Page unavailable")}</span></div>{chunk.heading && <h3>{chunk.heading}</h3>}<details><summary>{language === "vi" ? "Đọc đoạn văn" : "Read indexed passage"}</summary><p>{chunk.content}</p></details></article>)}</div> : <Empty title={language === "vi" ? "Chưa có đoạn được lập chỉ mục" : "No indexed chunks"} text={language === "vi" ? "Văn bản này chưa tạo ra đoạn văn có thể tìm kiếm." : "This document has not produced searchable passages yet."} />}
  </>;
}
