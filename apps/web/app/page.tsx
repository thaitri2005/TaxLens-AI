...
function DocumentDetailView({ detail, chunks }: { detail: DocumentDetail; chunks: Chunk[] }) { return <><Heading eyebrow="Indexed source" title={detail.document_number} chip={detail.document_type} /><p className="document-title">{detail.title}</p><div className="version-grid">{detail.versions.map(version => <div className="version-card" key={version.id}><span className="tag">{version.legal_status}</span><strong>Version {version.issue_date || "date unavailable"}</strong><small>Effective: {version.effective_date || "not recorded"}</small></div>)}</div><div className="chunk-heading"><p className="eyebrow">Indexed passages</p><span>{chunks.length} chunks</span></div>{chunks.length ? <div className="chunk-list">{chunks.map(chunk => <article className="chunk-card" key={chunk.id}><div className="chunk-top"><strong>{chunk.article_number ? `Article ${chunk.article_number}` : "Document passage"}</strong><span>{chunk.page_start ? `Page ${chunk.page_start}${chunk.page_end && chunk.page_end !== chunk.page_start ? `–${chunk.page_end}` : ""}` : "Page unavailable"}</span></div>{chunk.heading && <h3>{chunk.heading}</h3>}<p>{chunk.content}</p></article>)}</div> : <Empty title="No indexed chunks" text="This document has not produced searchable passages yet." />}</>; }
