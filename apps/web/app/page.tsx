...
export default function Home() {
  ...
  return <>
    ...
    <main className="shell">
      ...
      {view === "documents" && <section className="view">{selectedDocument ? <><button className="back-button" onClick={() => setSelectedDocument(null)}>← Back to documents</button>{documentBusy ? <Empty title="Loading document" text="Reading versions and indexed passages." /> : documentError ? <div className="error-state">{documentError}</div> : documentDetail && <DocumentDetailView detail={documentDetail} chunks={documentChunks} />}</> : <><Heading eyebrow="Source library" title="Documents" chip={`${documents?.length || 0} indexed`} />{documentError ? <div className="error-state">{documentError}</div> : documents === null ? <Empty title="Loading source library" text="Reading the indexed legal documents." /> : <div className="document-list">{documents.length ? documents.map(document => <button className="document-card document-button" key={document.document_number} onClick={() => openDocument(document)}><div className="doc-top"><span className="doc-number">{document.document_number}</span><span className="tag">{document.document_type}</span></div><strong>{document.title}</strong><div className="doc-meta">{document.issuing_agency || "Agency unavailable"}<span>Open details →</span></div></button>) : <Empty title="No documents indexed yet" text="Ingest official documents to populate the source library." />}</div>}</>}</section>}
