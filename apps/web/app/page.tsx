...
type DocumentDetail = Document & { id: string; versions: { id: string; issue_date: string | null; effective_date: string | null; legal_status: string; raw_artifact_key: string; processing_status: string | null; processing_error_code: string | null; chunk_count: number }[] };
...
export default function Home() {
  ...
  return <>
    ...
    <main className="shell">
      ...
      {view === "ask" && <section className="view"><Heading eyebrow="Evidence-grounded answers" title="Ask TaxLens" chip="Cited response" /><form onSubmit={ask} className="question-form"><textarea value={question} onChange={event => setQuestion(event.target.value)} required maxLength={1000} placeholder="Ask a question about Vietnamese tax regulations…" /><div className="form-actions"><span>Answers are generated from retrieved legal passages.</span><button type="submit">{answerBusy ? "Working…" : "Ask question →"}</button></div></form><ResultState error={answerError} hasResults={answer !== null} emptyTitle="Ask a focused question" emptyText="TaxLens will show its evidence status, answer, uncertainties, and citations." />{answer && <div className="answer-card"><span className="answer-status">{answer.status}</span>{answer.confirmed_facts.length > 0 && <div className="confirmed-facts"><p className="eyebrow">Confirmed facts</p>{answer.confirmed_facts.map(fact => <p key={fact.text}>{fact.text} <span className="fact-citations">[{fact.citation_numbers.join(", ")}]</span></p>)}</div>}<div className="answer-text">{escapeText(answer.answer || answer.interpretation)}</div>{answer.uncertainties.length > 0 && <div className="uncertainties"><strong>Uncertainties</strong><br />{answer.uncertainties.map(item => <span key={item}>{item}<br /></span>)}</div>}{answer.review_actions.length > 0 && <div className="review-actions"><strong>Review actions</strong>{answer.review_actions.map(action => <span key={action}>• {action}</span>)}</div>}<div className="answer-citations"><p className="eyebrow">Sources</p>{answer.citations.map((citation, index) => <CitationLine key={`${citation.document_number}-${index}`} citation={citation} />)}</div><p className="disclaimer">{answer.disclaimer}</p></div>}</section>}
      {view === "compare" && <section className="view"><Heading eyebrow="Version intelligence" title="Compare regulations" chip="Article diff" /><form onSubmit={compare} className="compare-form"><label>Before document<input value={before} onChange={event => setBefore(event.target.value)} list="document-options" required placeholder="e.g. 02/2024/TT-BTC" /></label><span className="swap">→</span><label>After document<input value={after} onChange={event => setAfter(event.target.value)} list="document-options" required placeholder="e.g. 31/2025/TT-BTC" /></label><button type="submit">{compareBusy ? "Comparing…" : "Compare versions →"}</button><datalist id="document-options">{documents?.map(document => <option key={document.document_number} value={document.document_number}>{document.title}</option>)}</datalist></form><ResultState error={compareError} hasResults={changes !== null} emptyTitle="Choose two document versions" emptyText="Use document numbers from the Documents tab to see added, removed, and modified articles." />{changes && <div className="comparison-list">{changes.length ? changes.map(change => <article className={`change-card ${change.change_type.toLowerCase()}`} key={change.key}><div className="change-top"><strong>{change.key}</strong><span className="change-type">{change.change_type}</span></div><div className="change-content"><div><strong>Before</strong>{change.before_content || "—"}{change.before_citation && <CitationLine citation={change.before_citation} />}</div><div><strong>After</strong>{change.after_content || "—"}{change.after_citation && <CitationLine citation={change.after_citation} />}</div></div></article>) : <Empty title="No article changes found" text="The selected versions did not produce a detected article difference." />}</div>}</section>}
      {view === "documents" && <section className="view">{selectedDocument ? <><button className="back-button" onClick={() => setSelectedDocument(null)}>← Back to documents</button>{documentBusy ? <Empty title="Loading document" text="Reading versions and indexed passages." /> : documentError ? <div className="error-state">{documentError}</div> : documentDetail && <DocumentDetailView detail={documentDetail} chunks={documentChunks} />}</> : <><Heading eyebrow="Source library" title="Documents" chip={`${visibleDocuments?.length || 0} indexed`} /><div className="form-row"><label>Source<select value={documentSourceFilter} onChange={event => setDocumentSourceFilter(event.target.value)}><option value="all">All sources</option>{Array.from(new Set(documents?.map(document => document.source_name).filter(Boolean))).map(source => <option key={source} value={source || ""}>{source}</option>)}</select></label><label>Type<select value={documentTypeFilter} onChange={event => setDocumentTypeFilter(event.target.value)}><option value="all">All types</option>{Array.from(new Set(documents?.map(document => document.document_type))).map(type => <option key={type} value={type}>{type}</option>)}</select></label></div>{documentError ? <div className="error-state">{documentError}</div> : documents === null ? <Empty title="Loading source library" text="Reading the indexed legal documents." /> : <div className="document-list">{visibleDocuments?.length ? visibleDocuments.map(document => <button className="document-card document-button" key={document.document_number} onClick={() => openDocument(document)}><div className="doc-top"><span className="doc-number">{document.document_number}</span><span className="tag">{document.document_type}</span></div><strong>{document.title}</strong><div className="doc-meta">{document.issuing_agency || "Agency unavailable"}<span>{document.source_name || "Source unavailable"} · Open details →</span></div></button>) : <Empty title="No matching documents" text="Adjust the source or document type filters." />}</div>}</>}</section>}
...
function DocumentDetailView({ detail, chunks }: { detail: DocumentDetail; chunks: Chunk[] }) {
  return <>
    ...
    <p className="document-source">{detail.source_name || "Source unavailable"} · {detail.issuing_agency || "Agency unavailable"}</p>
    ...
    <div className="chunk-heading"><p className="eyebrow">Indexed passages</p><span>{chunks.length} chunks</span></div>
    {chunks.length ? <div className="chunk-list">{chunks.map(chunk => <article className="chunk-card" key={chunk.id}><div className="chunk-top"><strong>{chunk.article_number ? `Article ${chunk.article_number}` : "Document passage"}</strong><span>{chunk.page_start ? `Page ${chunk.page_start}${chunk.page_end && chunk.page_end !== chunk.page_start ? `–${chunk.page_end}` : ""}` : "Page unavailable"}</span></div>{chunk.heading && <h3>{chunk.heading}</h3>}<p>{chunk.content}</p></article>)}</div> : <Empty title="No indexed chunks" text="This document has not produced searchable passages yet." />}
  </>;
}
