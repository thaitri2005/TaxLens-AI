...
export default function Home() {
  ...
  return <>
    <header className="topbar"><a className="brand" href="#search"><span className="brand-mark">TL</span><span><strong>TaxLens</strong><small>{language === "vi" ? "Thông tin pháp lý" : "Regulatory intelligence"}</small></span></a><div className="topbar-actions"><div className="language-toggle" aria-label="Language"><button className={language === "en" ? "active" : ""} onClick={() => setLanguage("en")}>EN</button><button className={language === "vi" ? "active" : ""} onClick={() => setLanguage("vi")}>VI</button></div>{session?.user.role === "admin" && <a className="topbar-link" href="/admin">{language === "vi" ? "Quản trị" : "Admin"}</a>}<button className="sign-out-button" type="button" onClick={() => signOut({ callbackUrl: "/login" })}>{language === "vi" ? "Đăng xuất" : "Sign out"}</button><div className="status"><span className="status-dot" />Local workspace</div></div></header>
